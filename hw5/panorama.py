...
def simple_descriptor(patch):
    ...
    feature = []
    ### YOUR CODE HERE
    #computing the standart deviation of patch
    standart_deviation = patch.std()
    #since in the formula below the standart deviation is the denominator, so we set it to 1 if it is equal to 0
    if standart_deviation == 0:
        standart_deviation = 1
    #computing the mean of patch    
    mean = patch.mean()
    #applying formula
...
def match_descriptors(desc1, desc2, threshold=0.5):
    ...
    for x, dist in enumerate(dists):
        # the output is an array where each row holds the indices of one pair of matching descriptors.
        # this is why we used argmin to get returned the indices of the matching pair
        indice = np.argmin(dist)
        # we sort the array to keep comparing the two closest vectors
        sorted_distances = np.sort(dist)
        # if the ratio of the distances is smaller than the threshold append the result to the ouptut array
        if sorted_distances[0] / sorted_distances[1] < threshold:
            matches.append([x, indice])
    # convert the results to an array using asarray() function 
...
def fit_affine_matrix(p1, p2):
    ...
    ### YOUR CODE HERE
    # The np.linalg.lstsq() function returns the least-squares solution to a linear matrix equation.
    # It computes the vector x that approximately solves the equation a * x = b
    # exactly what we want : find H such that p2 * H = p1
